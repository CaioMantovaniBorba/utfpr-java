...
public class Teste {
	...
	public static void menuInicial(Leitura scan) throws IOException {
		...
		while (input != 7) {
			...
			System.out.println("Sistema de Gestao de Veiculos\n");
			...
			try {
				input = Integer.parseInt(scan.entDados("Digite uma das opcoes acima: "));
				opcoesMenu(scan, input, veiculosPasseio, veiculosCarga);
			} catch (NumberFormatException e) {
				System.out.println("A opcao escolhida deve ser um numero inteiro.");
				scan.entDados("");
				continue;
			}
		}
	...
	public static void opcoesMenu(Leitura scan, int opcao, Passeio[] veiculosPasseio, Carga[] veiculosCarga) {
		...
		case 7:
			break;
		default:
			System.out.println("opcao inválida. Escolha uma opcao de 1 a 7.");
		}
		...
		do {
			...
			try {
				...
				if (veiculo == null) {
					...
					System.out.println("Este Veiculo já está cadastrado.");
					return veiculosPasseio;
				}

				input = scan.entDados("Deseja adicionar mais Veiculos de passeio? (SIM/NAO)");

				if (input.equals("SIM") && isPasseioCheio(veiculosPasseio)) {
					System.out.println("\nA lista de Veiculos de passeio está cheia.\n");
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public static Carga[] cadastraVeiculoCarga(Leitura scan, Carga[] veiculosCarga) {
		...
		do {
			...
			try {
				...
				input = scan.entDados("Deseja adicionar mais Veiculos de carga? (SIM/NAO)");

				if (input.equals("SIM") && isCargaCheio(veiculosCarga)) {
					System.out.println("\nA lista de Veiculos de carga está cheia.\n");
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
